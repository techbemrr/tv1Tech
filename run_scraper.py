...
def get_gc():
    """Uses direct Credentials to bypass local cache/PermissionErrors."""
    scopes = [
...
def safe_call(func, *args, **kwargs):
    """Retries with exponential backoff and memory clearing on error."""
    for attempt in range(7):
        try:
            return func(*args, **kwargs)
        except Exception as e:
            err = str(e).lower()
            # If we get a Response 200 or Quota error, wait and retry
            if "200" in err or "429" in err or "quota" in err or "permission" in err:
                wait_time = (2 ** attempt) * 12 + random.uniform(2, 5)
                log(f"⚠️ Connection/Quota hit. Retrying in {wait_time:.1f}s...")
...
def connect_sheets():
    ...
    gc = get_gc()
    # Using open() by name - ensure the Service Account has 'Editor' access
...
def get_visible_value_elements(drv):
    # Using Javascript for faster, more reliable extraction of indicator values
    script = """
...
try:
    ...
    # Read entire columns to save read-requests
...
try:
    ...
    restart_driver()
    log("🏁 Shard Completed.")import sys
import os
import time
import json
import random
